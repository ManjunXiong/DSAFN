...
def _make_data_and_model(args):
    ...
    model = MFDAE(filters=[32, 64, 128, 10], n_clusters=n_clusters, view_shape=view_shapes)
...
def train(args):
    ...
    if args.pretrain_dir is not None and os.path.exists(args.pretrain_dir):  # load pretrained weights
       ...
       # scio.savemat('data' + "1" '.mat', {'Z': x, 'Y': y, 'P': y_mean_pred})
...
if __name__ == "__main__":
    ...
    data = 'N_CWRU_V2'#换数据集修改此处数据集名字即可
    ...
    #########
    epochs = 10 # 10 epochs for pre-train AEs
    Update_Coo = 2000
    #Update_Coo = 200   # iterations to update self-supervised objective

    Maxiter = 2000 # Max iterations for DEMVC#200
    Batch = 16 # Batch size#
    Idec = 1
